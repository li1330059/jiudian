<template>
	<view class="myOrder" :style="{ height: screenHeight }">
		...
		<view class="hdd">
				...
				<view style="font-size: 34rpx; margin-left: 240rpx;">我的订单</view>	

			</view>	
			<view style="margin-top: 80rpx;">
				<ul>
					<li class="lli" style="display: inline-block; font-size: 26rpx; margin-left: 7%;"  v-for="(item,index) in tabs1":key="index" @click="btnn(index,item.status)" :class="{'active1' : current1==index}">{{item.value}}</li>
				</ul>
			</view>			
		</view>
								...
								<view class="jpkcContent" v-if="works.length!=0">
									<ul>
										<li   v-for="(item,index) in works" :key="index">
											...
											<view class="jpkcLeft">
												<view class="jpkcImg" v-if="item.dd != 2">
													<image style="width:160rpx; height:160rpx; border-radius: 20rpx;" :src="item.tp"></image>
												</view>
												<view class="jpkcImg" v-if="item.dd == 2">
													<image style="width:160rpx; height:160rpx; border-radius: 20rpx;" :src="item.arrs[0]"></image>
												</view>
												...
												<view class="jpkczi">
													<view style="width: 450rpc; display: flex; justify-content: space-between;">
														...
														<view v-if="item.status==1" style="color: #22e015;">已付款</view>
														 <view v-if="item.status==2 && item.tkstatus==0" style="color: #e02433;">退款失败</view>
														 <view v-if="item.status==2 && item.tkstatus==1" style="color: #e02433;">待审核</view>
														 <view v-if="item.status==2 && item.tkstatus==2" style="color: #22e015;">审核通过</view>
														 <view v-if="item.status==2 && item.tkstatus==3" style="color: #e02433;">审核未通过</view>
													...
													<view class="jpkcBottom">
														...
														<view style="margin-top: 18rpx; color: #adadad; font-size: 24rpx;" class="jpkcPrice">
															...
															<view v-if="item.status==2 && item.tkstatus==3">
																<view class="ydd" @click="wtg(item.ddhao)" >查看详情</view>
															</view>
															...
															<neil-modal class="nei" :show="show1"  @close="closeModal()" @confirm="clickRight">
																<view  style="width: 90%; margin: 20rpx auto;">
																	<view >
																		<view style=" text-align: center;">退款原因</view>
																		<view>{{wtgyy}}</view>	
																	</view>
																</view>
...
<script>
	...
	import neilModal from "../../components/neil-modal/neil-modal1.vue"
	export default {
		components:{ neilModal, serviceDialog },
		data() {
			return {
				//屏幕高度
				screenHeight: 0,
			...
			hx(id,dd){
				uni.request({
					...
					success: (res) => {
						...
						if(this.orderInfo.jdName==undefined){
							this.orderInfo["jdName"]=this.orderInfo.tclxname
						}
						console.log(this.orderInfo)
						uni.navigateTo({
				...
				uni.request({
					...
					success: (res) => {
						...
						if(this.orderInfo.dd==2){
							this.orderInfo["arrs"]=this.orderInfo.tp.split("|")
						}
						console.log(this.orderInfo)
					}
				})
					...
					success: (res) => {
						...
						//if(dd==2){
							for(let i=0; i<this.works.length; i++){
								if(this.works[i].dd==2){
									this.works[i]["arrs"]=this.works[i].tp.split("|")
								}
