<template>
	<view class="qrr" :style="{ height: screenHeight }">
		...
		<view class="qbody">
			<view class="qToo">
				<image style="width: 160rpx; height: 160rpx; border-radius: 20rpx;" :src="tclxs.tp"></image>
				<view style="margin-left: 20rpx;  width: 500rpx;">
					<view style="font-weight: bold;">{{tclxs.title}}</view>
					<view style="font-size: 24rpx; color: #868686; margin-top: 20rpx;">{{tclxs.value}}</view>
					<view class="price_num">
						<view class="spjine">
							￥<span style="font-size: 32rpx;">{{tclxs.dj}}</span>
						</view>
						<view class="numOpt">
			...
			<!-- 	<view class="qrow">
				...
				<view class="qrow">
					...
					<view style="margin-left: 40rpx;">
						<input  placeholder="请写入出行人姓名" v-model="liverName" />
					</view>
				</view>
				<view class="qrow">
					<view style="color: #858585; width: 180rpx;">身份证</view>
					<view style="margin-left: 40rpx;">
						<input  placeholder="请写入出行的证件号码" v-model="IdCard" />
					</view>
				</view>
				<view class="qrow">
...
<script>
	...
	export default {
		...
		onShow() {
			uni.request({
				method:"POST",
				url:this.oldApiUrl+"index/tclxxq",
				data:JSON.stringify({
					id:this.tclxid
				...
				if(this.liverName!="" && IDCard_RGEX.test(this.IdCard)){
					...
					uni.request({
						...
						success: (res) => {
							...
							uni.request({
								method:"POST",
								url:this.oldApiUrl+"index/ddxqddh2",
								data:JSON.stringify({
									"ddhao":this.ddhao
								}),
								success: (rres) => {
											...
											console.log(res)
														...
														uni.requestPayment({
															...
															success: function (res) {
																...
																// uni.request({
																// 	method:"POST",
																// 	url:this.oldApiUrl+"index/wxback",
																// 	data:JSON.stringify({
																// 		"id":this.iddd+1,
																// 		"user_name":uni.getStorageSync("openid")
																// 	}),
																...
																// uni.request({
																// 	method:'POST',
																// 	url:this.oldApiUrl+"index/find_user_code",
																// 	data:JSON.stringify({
